"""
From https://github.com/jquesnelle/yarn/blob/master/scaled_rope/LlamaYaRNScaledRotaryEmbedding.py
...
class LlamaYaRNScaledRotaryEmbedding(torch.nn.Module):
    def __init__(
        ...
        t = torch.arange(self.max_seq_len_cached, device=self.inv_freq.device, dtype=self.inv_freq.dtype,) # type: ignore
    ...
    def forward(self, x, seq_len=None):
        ...
        if seq_len > self.max_seq_len_cached: # type: ignore
            self.max_seq_len_cached = seq_len

            t = torch.arange(self.max_seq_len_cached, device=x.device, dtype=self.inv_freq.dtype,) # type: ignore
            freqs = torch.einsum("i,j->ij", t, self.inv_freq)
            # Different from paper, but it uses a different permutation in order to obtain the same calculation
